...
1. [Criar Tabela tbwhatsapp_send](#1-criar-tabela-tbwhatsapp_send)
...
WHERE table_name = 'tbwhatsapp_send';
...
```sql
-- Listar todos os índices da tabela tbwhatsapp_send
SELECT 
  indexname,
  indexdef
FROM pg_indexes
...
SELECT * FROM tbwhatsapp_send ORDER BY created_at DESC LIMIT 10;
...
SELECT * FROM tbwhatsapp_send LIMIT 1;
```
